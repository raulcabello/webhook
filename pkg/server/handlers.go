...
func Validation(clients *clients.Clients) ([]admission.ValidatingAdmissionHandler, error) {
	...
	if clients.MultiClusterManagement {
		...
		clusterRoleBindins := clusterrolebinding.NewValidator()

		handlers = append(handlers, psact, globalRoles, globalRoleBindings, prtbs, crtbs, roleTemplates, secrets, nodeDriver, projects, roles, rolebindings, clusterRoles, clusterRoleBindins)
	}
	return handlers, nil
}
